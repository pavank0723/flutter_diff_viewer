...
### Added
- **Visual Added Whitespace Highlighting (`highlightWhitespace`)**: Added `highlightWhitespace` flag in `FlutterDiffViewerConfiguration` (defaults to `true`) to visually highlight added/extra whitespace characters (`·` for spaces, `→` for tabs).
- **Customizable Whitespace Color Tokens**: Added `addedWhitespaceBackgroundColor` and `addedWhitespaceTextColor` in `FlutterDiffViewerTheme` with vibrant default badge styling to make added/extra whitespace stand out clearly.
- **Synchronized Full-View Horizontal Scrolling (`DiffHorizontalScrollSync`)**: Unified horizontal scrolling across all line rows in a panel so lines scroll together smoothly without single-line drift or layout performance bottlenecks.
